...
// Classes for transient track for impact parameter (ip3d, sip3d).
#include "TrackingTools/TransientTrack/interface/TransientTrackBuilder.h"
#include "TrackingTools/Records/interface/TransientTrackRecord.h"
#include "TrackingTools/TransientTrack/interface/TransientTrack.h"
#include "TrackingTools/IPTools/interface/IPTools.h"
//#include "DataFormats/GeometryCommonDetAlgo/interface/Measurement1D.h" //already included in "IPTools.h"
...
class Muon_Analyzer : public edm::one::EDAnalyzer<>  {
   ...
   private:
      ...
      std::vector<float> muon_dxy; // to get info associated to the track of the muon
      std::vector<float> muon_dz; // to get info associated to the track of the muon
      std::vector<float> muon_dxyError; // to get info associated to the track of the muon
      std::vector<float> muon_dzError; // to get info associated to the track of the muon
      std::vector<float> muon_pfreliso03all;
      std::vector<float> muon_pfreliso04all;
      std::vector<double> muon_pfreliso04DBCorr;
      std::vector<float> muon_TkIso03;
      std::vector<float> muon_genpartidx;
      std::vector<float> muon_jetidx;
      std::vector<double> muon_ip3d;
      std::vector<double> muon_sip3d;
