...
Electron_Analyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   ...
   //or (make a check to see types)
   //const reco::Vertex primaryVertex = vertices->at(0); // but wee need a refernce, does it work? Yes :).
   //std::cout<< "PV: " << primaryVertex << std::endl;
   ...
   // "electrons" is a pointer to a vector of "pat::Electron" object refernces?
   for(const pat::Electron& el : *electrons){
     ...
     //reco::GsfTrackRef track = el.gsfTrack();
...
Electron_Analyzer::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  descriptions.add("electronAnalyzer", desc);
  //desc.setUnknown();
